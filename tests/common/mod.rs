...
unsafe fn run_party(mut stream: UnixStream, args: Vec<String>, p: c_int, run_fn: ProtocolFn) {
...
pub unsafe fn run_test(args: Vec<String>, run_fn: ProtocolFn) {
    ...
    let server = thread::spawn(move || run_party(stream1, server_args, 1, run_fn));
    run_party(stream2, args, 2, run_fn);
    server.join().unwrap();
}
